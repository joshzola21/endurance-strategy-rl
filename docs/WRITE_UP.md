...
*A reinforcement learning project that failed at what it set out to do, and
found four better things on the way.*

---

## The problem

In a twenty-four-hour race, the fastest car does not always win. The car that
stops at the right moments does.

Endurance racing is decided in the pit lane. A stop costs a minute and a half
and you will take twenty-five of them; whether each one costs you a place
depends on what the rest of the field is doing, on whether a caution period is
about to bunch everyone up, and on whether the pit lane is even open to your
class when you arrive. Get the timing right and you gain a lap on the field for
free. Get it wrong and you spend the next four hours recovering.

That makes it a decision problem with a shape reinforcement learning ought to
like: a long sequence of small choices, a delayed and noisy payoff, and a
strategic structure that is genuinely hard for a person to reason about at the
scale of a whole race.

I wanted to know whether a learned policy could beat human-style pit strategy.
It could not, and the reason turned out to be far more interesting than the
question.
...
- **four measured ways this simulator misleads a learner**, each measured
  directly on the engine rather than inferred from how a policy scored
- an evaluation protocol that can tell a policy which has learned something
...
None of those numbers is a bug in the arithmetic. They are all the same
mistake: the query scoping the data to "the Daytona 24" was matching on the
event name, and the file holds nine runnings of it. I was calibrating a race
that was nine Daytonas stacked on top of each other. The lap counter ran
straight through them, so a "stint" spanned editions; the duration was nine
races long; and pooling nine grids of different cars destroyed the relationship
between tyre age and lap time that the degradation fit depends on.

The fix was one column — the file has a `session_id`, and one race is one
session. But finding it meant profiling the file rather than trusting it, and
that turned up three more things of the same kind:
...
and one car vanishes from the grid.

**The stint counter does not count stints.** It steps on driver changes, which
happen about every third stop. Reading it as a fuel stint reported 58-lap green
...
Each of these produced output that looked entirely plausible. None of them
raised an error. That is the whole lesson of the data stage, and it is why
every later stage in this project carries a check that is designed to *fail* on
a specific fault rather than merely to pass.
...
a mean. And at Daytona the front-running classes change tyres at every stop,
which makes tyre age and fuel load the same number — no fit can separate them,
so the calibration reports the degradation slope as *unidentified* rather than
returning a figure it cannot support. At Le Mans the fitted slope for Hypercar
and LMP2 comes out **negative**, which cannot be tyre wear. It is fuel burning
off and the track rubbering in, and the data cannot pull them apart. The
simulator applies it as though it were tyre wear, and the app says so on every
screen.
...
traffic density. Everything else is derived.

The split that matters is not between dials — it is between **what the timing
data can measure and what it cannot**. Lap timing records that a lap was slow;
it never records how close the car ahead was. It records how long a stop took;
it never records what happened during it. So a set of quantities — how much a
caution slows the field, how tightly cars bunch behind a safety car, how a stop
divides between driving down the lane, changing tyres and putting fuel in — are
**assumed**, marked as such everywhere they appear, and exposed as sliders in
the app.

That is not a disclaimer. It is the design. The honest response to a number
somebody chose is to move it and see whether the conclusion survives, so every
assumed dial is swept rather than trusted.

**One statement sits above all of them.** These dials come from *one running of
one race per series* — Daytona 2026 and Le Mans 2026. Between two adjacent
Daytonas the share of the race spent under caution moves by a factor of 2.8. So
the width of each slider is a better guide to the uncertainty than its starting
value is.
...
forbids tools during refuelling, so the jobs are sequential and a stop costs
their *sum*.
...
gains a place in **58% of 200 races** at Daytona and is worth much less at Le Mans,
entirely because of who gets released from a closed pit lane and when.
...
strategy that got five good races.
...
The difference between those two runs is the strategy's effect on *that* race,
...
That only works if the noise is a property of the race rather than of the
strategy — and at first it was not. Under a single shared random generator, a
car that pitted earlier drew different lap-time noise for the rest of the race,
so a comparison measured the strategy *and* a reshuffle. Now every random
number is drawn from the seed before the race starts, and each car's noise
lives in an indexed stream, so a car's third pit stop gets its third noise draw
whenever it takes it. Two strategies stopping at completely different moments
see identical noise on identical laps.
...
of 200 races**, with a median gain of one place, and it holds up on fifty
races the strategy was never selected on. It is the strongest single result the
project has.
...
The policy sees ten numbers at each crossing of the line — race progress, fuel,
tyre age, the gaps ahead and behind, the flag, stint length, laps down, whether
the lane is open, and its class position — and chooses among five actions:
stay out, or one of four combinations of fuel and tyres. It is trained with
masked PPO, and it is scored by being inserted into the roster as a seventh
member and run through exactly the same function the six others go through.
There is deliberately no agent-specific evaluation code: a second path that can
differ from the roster's produces plausible numbers rather than errors.
...
hundred paired races can resolve. The five policies were not variations on one
behaviour: their median stop counts ran 38, 65, 72, 174, 194 in a race where a
sensible number is about forty.

**In IMSA it was worse, because it looked stable.** All five seeds came out
identical to three decimal places on every column. A policy that had learned
something would vary. So I built `never_pit` — a strategy that always stays out
and lets the engine's out-of-fuel rule supply every stop — and scored it against
the roster.

It scored the same as the trained policy. On the fifty held-out races the two
...
produce five identical races. What looked like a stable result was a policy
that had learned to take no decisions.

`never_pit` is now the sixth member of the roster, and it is the true null for
a learner: a policy that cannot beat *taking no decisions* has learned nothing.
...
the policy anywhere in the app. It can still be watched taking decisions lap by
...
slow" was being applied to the drive down the lane as well. The policy found
it and stopped 164 times in a 189-lap race. That is not a bug in the policy; it
is the policy being right about the model.
...
**3. The credit horizon is shorter than the payback window.** A forced extra
...
At Daytona the sign flips. The learning algorithm's credit assignment puts
essentially all of the weight inside about twenty steps, so the agent was told
the truth about the next twenty laps and the opposite about the race. At Le
...
cheaper than a full service, and it is the number the policy exploited in trap
...
with a planted answer of 0.407, that estimator returns 0.548. It is biased
high by about a seventh, in the direction you would expect: the cheapest stops
in any real sample still have *some* service in them.
...
rather than the method: **endurance cars fill to the brim.** Between
62% and 90% of stops in these two races are followed by a near-full tank, and the
...
The first reported that 95% of a stop was fixed cost — an artefact, because a
...
converted the failed fit into a lower bound of 84% to 98% — also withdrawn,
because it assumed a short stint means a small fill, and a stint also ends
early for a caution or for the flag, with nothing in the data to say which.
...
**Train for races, not laps.** The budget is counted in decision steps, and
Daytona costs 742 of them per race against Le Mans's 385 — so the same budget
buys 674 Daytonas and 1,299 Le Mans. That asymmetry ran through the whole
project untested.

**Report an agent as a distribution.** Two hundred paired races exist because
one race means nothing; a single training run is the same mistake one level up.
Five seeds should be the floor, and the stop count should be reported beside
the score, because two runs can score alike for entirely different reasons.

**Rank the four traps.** Each is measured, but which dominates cannot be
settled without training runs that resolve, and at half a million steps they do
not.
...
exit are most of the cost whatever you stopped for.
